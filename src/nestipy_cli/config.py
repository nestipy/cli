...
def build_granian_log_dictconfig(
    ...
    formatter: dict[str, Any] = {
        "()": "nestipy_cli.config._NestipyRichFormatter",
        "fmt": fmt,
        "datefmt": datefmt,
    }
    if not use_color:
    ...
    if use_color:
        handler: dict[str, Any] = {
            "()": "nestipy_cli.config._NestipyRichHandler",
            "level": level,
            "formatter": "nestipy",
            "rich_tracebacks": True,
            "markup": True,
    ...
    return {
        ...
        "loggers": {
            ...
            "_granian": {"handlers": ["console"], "level": level, "propagate": False},
            "nestipy": {"handlers": ["console"], "level": level, "propagate": False},
        },
        "root": {"handlers": ["console"], "level": level},
    }
log_dir = os.path.join(os.getcwd(), "logs")
_BASE_GRANIAN_LOG_CONFIG = build_granian_log_dictconfig()
LOGGING_CONFIG: dict[str, Any] = {
    **_BASE_GRANIAN_LOG_CONFIG,
    "formatters": {
        **_BASE_GRANIAN_LOG_CONFIG["formatters"],
        "file": {"class": "logging.Formatter", "format": DEFAULT_LOG_FORMAT},
    },
    "handlers": {
        **_BASE_GRANIAN_LOG_CONFIG["handlers"],
        "default": {
